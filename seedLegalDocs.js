...
const policies = [
  {
    ...
    content: `UBS GLOBAL IMPORTING & EXPORTING - PRIVACY POLICY
...
• Response Time: We commit to reviewing and responding to all verified inquiries within thirty (30) days of receipt.`
  },
  {
    ...
    content: `UBS GLOBAL IMPORTING & EXPORTING - TERMS & CONDITIONS
...
• Legal Enquiries: legal@ubsglobal.com`
  },
  {
    key: 'refund-policy',
    title: 'Refund Policy',
    content: `UBS GLOBAL IMPORTING & EXPORTING - REFUND POLICY
Last Updated: June 2025
...
• In-App Support: Support ticket chat`
  },
  {
    ...
    content: `UBS GLOBAL IMPORTING & EXPORTING - ACCOUNT DELETION POLICY
Last Updated: June 2025
...
const seed = async () => {
  try {
    const mongoUri = process.env.MONGO_URI || 'mongodb://localhost:27017/ubsglobal'
    console.log('Connecting to database for seeding compliance policies...', mongoUri)
    await mongoose.connect(mongoUri)

    console.log('Clearing existing legal documents...')
    await LegalDoc.deleteMany({})

    console.log('Seeding compliance documents...')
    await LegalDoc.insertMany(policies)

    console.log('Legal documents seeded successfully!')
    process.exit(0)
  } catch (error) {
    console.error('Seeding legal docs failed:', error)
    process.exit(1)
  }
}
