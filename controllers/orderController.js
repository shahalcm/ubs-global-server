...
async function processShiprocketOrderPipeline(order, seller) {
  try {
    ...
    const totalWeight = order.items.reduce((acc, i) => acc + (0.5 * (i.quantity || 1)), 0)
    ...
    const email = (addr.email || 'customer@ubsglobal.com').trim()
    const rawPhone = (addr.phone || '9999999999').trim().replace(/\D/g, '')
